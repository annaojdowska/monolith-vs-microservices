...
public class RouteServiceImpl extends RouteServiceProtoGrpc.RouteServiceProtoImplBase {

  @Override
  public void findShortestRoute(Route.FindRouteMessage request, StreamObserver<Route.ShortRoute> responseObserver) {

    Point[] input = GetRandomPoints(10000);
    ArrayList<Point> route = FindRoute(input);
    List<String> collect = route.stream().map(x -> x.getName()).collect(Collectors.toList());

    Route.ShortRoute response = Route.ShortRoute.newBuilder().addAllPoint(collect).build();
